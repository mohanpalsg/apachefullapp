...
<c:import url="aftertbodybeforecriteria.txt" />
 ...
 <select id="W4LevelSelect" onchange="Currentweeksmapvselect()" multiple="multiple">
    <option   value="PP"  <c:if test="${cCurrentWeeksma50pvPP eq true}">
selected
</c:if>>Pivot PP</option>
    <option value="S1" <c:if test="${cCurrentWeeksma50pvS1 eq true}">
selected
</c:if>>Pivot S1</option>
    <option value="S2" <c:if test="${cCurrentWeeksma50pvS2 eq true}">
selected
</c:if>>Pivot S2</option>
    <option value="S3" <c:if test="${cCurrentWeeksma50pvS3 eq true}">
selected
</c:if>>Pivot S3</option>
    <option value="S4" <c:if test="${cCurrentWeeksma50pvS4 eq true}">
selected
</c:if>>Pivot S4</option>
    <option value="R1" <c:if test="${cCurrentWeeksma50pvR1 eq true}">
selected
</c:if>>Pivot R1</option>
     <option value="R2" <c:if test="${cCurrentWeeksma50pvR2 eq true}">
selected
</c:if>>Pivot R2</option>
      <option value="R3" <c:if test="${cCurrentWeeksma50pvR3 eq true}">
selected
</c:if>>Pivot R3</option>
       <option value="R4" <c:if test="${cCurrentWeeksma50pvR4 eq true}">
selected
