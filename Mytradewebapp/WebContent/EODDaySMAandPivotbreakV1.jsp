...
<script type="text/javascript">
        function daysmapvselect() {
        	...
        	$.post(
                    "DaysmapvServlet", 
                    {nse200attr : $(wknse200check).is(':checked'),otherattr : $(wknseothercheck).is(':checked'),pivotdiffattr: $("#wksmpvval").val(), smadiffattr : $("#daysmapvsmval").val(),
...
<div class="panel panel-primary">
                            <div class="panel-heading">
                                <p>Dashboard EOD- Daily Pivot &amp; SMA Breaking Stock</p>
                            </div>
...
<c:forEach var="entry" items="${stocklist}" >
 ...
 <select id="W1LevelSelect" onchange="daysmapvselect()" multiple="multiple">
    <option id="Daysma50pvPPcheck"  value="PP"  <c:if test="${Daysma50pvPP eq true}">
selected
</c:if>>Pivot PP</option>
    <option value="S1" <c:if test="${Daysma50pvS1 eq true}">
selected
</c:if>>Pivot S1</option>
    <option value="S2" <c:if test="${Daysma50pvS2 eq true}">
selected
</c:if>>Pivot S2</option>
    <option value="S3" <c:if test="${Daysma50pvS3 eq true}">
selected
</c:if>>Pivot S3</option>
    <option value="S4" <c:if test="${Daysma50pvS4 eq true}">
selected
</c:if>>Pivot S4</option>
    <option value="R1" <c:if test="${Daysma50pvR1 eq true}">
selected
</c:if>>Pivot R1</option>
     <option value="R2" <c:if test="${Daysma50pvR2 eq true}">
selected
</c:if>>Pivot R2</option>
      <option value="R3" <c:if test="${Daysma50pvR3 eq true}">
selected
</c:if>>Pivot R3</option>
       <option value="R4" <c:if test="${Daysma50pvR4 eq true}">
selected
...
name="nse200"      
onchange = "daysmapvselect()" <c:if test="${nsechecked eq true}">
checked
</c:if>>nse200     
<input id="wknseothercheck" type="checkbox"     
name="others"     
onchange = "daysmapvselect()" <c:if test="${otherchecked eq true}">
checked
</c:if>/>Others  
...
Smadiff<input id="daysmapvsmslider" type="range" value="<c:out value="${smadiffval}"/>" min="0" max="10" step="0.5"  onchange="daysmapvselect()" oninput="daysmapvsmval.value = daysmapvsmslider.value">
<output name="daysmapvsmOutputName" id="daysmapvsmval"><c:out value="${smadiffval}"/></output> 
&nbsp;&nbsp;
<br>
pivotdiff<input id="daysmapvpivotslider" type="range" value="<c:out value="${pivotdiffval}"/>" min="0" max="10" step="0.5" onchange="daysmapvselect()" oninput="wksmpvval.value = daysmapvpivotslider.value">
<output name="daysmapvpivotsliderOutputName" id="wksmpvval"><c:out value="${pivotdiffval}"/></output>
<br>
</div>
</div>
